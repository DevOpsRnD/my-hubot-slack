...
TrelloExtended = require '../../../scripts/apps/lib/class.trello.js'
trello = new TrelloExtended process.env.HUBOT_TRELLO_KEY, process.env.HUBOT_TRELLO_TOKEN
...
createCard = (msg, list_name, cardName) ->
  ...
  id = trello.lists[list_name.toLowerCase()].id
...
createLabel = (msg, list_name, cardName, labelName) ->
  msg.reply "Sure thing boss. I'll create that card for you."
  ensureConfig msg.send
  id = trello.lists[list_name.toLowerCase()].id
  trello.post "/1/cards", {name: cardName, idList: id, labels: labelName}, (err, data) ->
    msg.reply "There was an error creating the card Label" if err
    msg.reply "OK, I created that card Label for you. You can see it here: #{data.url}" unless err

showCards = (msg, list_name) ->
  msg.reply "Looking up the cards for #{list_name}, one sec."
  ensureConfig msg.send
  console.log('Current Card')
  console.log(trello.lists[list_name.toLowerCase()])
  id = trello.lists[list_name.toLowerCase()].id
...
getMemberId = (msg, member_name) ->
  if (trello.members[member_name.toLowerCase()]?)
     id = trello.members[member_name.toLowerCase()].id
...
getMemberCards = (msg, member_name) ->
  if (trello.members[member_name.toLowerCase()]?)
     id = trello.members[member_name.toLowerCase()].username
  msg.send "Unable to find person named: #{user_name}" unless id?
  if id?
    trello.get "/1/search", {query: "@"+id, idBoards: trello.board.id, modelTypes: "cards", card_fields: "name,shortLink,url"}, (err, data) ->
...
search = (msg, search) ->
  if (search?)
    ...
    trello.get "/1/search", { query: search, idBoards: trello.board.id, modelTypes: "cards", card_fields: "name,shortLink,url" }, (err, data) ->
      if err
        msg.reply "Sorry, I was unable to search at this time. Please try again later."
        return

      if !data.cards.length
        msg.reply "Sorry, we didn't find anything with keyword #{search}."
        return
...
createList = (msg, list_name, position = "top") ->
  ...
  if position is "top" or position is "bottom" or position > 0
    trello.post "/1/lists", { name: list_name, idBoard: trello.board.id, pos: position }, (err, data) ->
      msg.reply "I'm sorry I was unable to add that list. Please try again later." if err
      msg.reply "That list has been created and be viewed here: #{trello.board.url}" unless err
...
module.exports = (robot) ->
  ...
  trello.get "/1/boards/#{process.env.HUBOT_TRELLO_BOARD}", (err, data) ->
    ...
    trello.get "/1/boards/#{process.env.HUBOT_TRELLO_BOARD}/lists", (err, data) ->
      for list in data
        trello.lists[list.name.toLowerCase()] = list
    trello.get "/1/boards/#{process.env.HUBOT_TRELLO_BOARD}/members", (err, data) ->
      for member in data
        trello.members[member.fullName.toLowerCase().split " ", 1] = member

  robot.respond /trello new ["“'‘](.+)["”'’]\s(.*)\s(.*)/i, (msg) ->
    ensureConfig msg.send
    label_name = msg.match[2]
    ...
    if(label_name !=''){
      createLabel msg, list_name, card_name, label_name
    }else{
      createCard msg, list_name, card_name
    }
    

    robot.respond /trello add label ["“'‘](.+)["”'’]\s(.*)\s(.*)/i, (msg) ->
    ensureConfig msg.send
    card_name = msg.match[2]
    list_name = msg.match[1]
    label_name = msg.match[3]
    if card_name.length == 0
      msg.reply "You must give the card a name"
      return
    ...
    if label_name.length == 0
      msg.reply "You must give a Label name"
      return
    return unless ensureConfig()
  ...
  # robot.send JSON.stringify({ "text": "Would you like to play a game?" })
  # robot.messageRoom "web-app", "I do not like green eggs and ham.  I do not like them sam-I-am."
  
  robot.respond /trello help/i, (msg) ->
    
    msg.reply "Here are all the commands for me."
    msg.send " *  trello new \"<ListName>\" <TaskName> <Labels>"
    msg.send " *  trello add label \"<ListName>\" <TaskName> <Labels>"
